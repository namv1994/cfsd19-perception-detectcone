...
double DetectCone::adjustLighting(cv::Mat img){
...
void DetectCone::forwardDetectionORB(cv::Mat img){
  ...
  for(size_t i = 0; i < point3Ds.size(); i++){
    int radius = xyz2xy(Q, point3Ds[i], point2D, 0.5f);
    int x = int(point2D.x);
    int y = int(point2D.y);
  ...
  if(inputs.size()>0){
    ...
    for(size_t i = 0; i < cones.size(); i++){
      ...
      int radius = xyz2xy(Q, point3D, position_tmp, 0.5f);
